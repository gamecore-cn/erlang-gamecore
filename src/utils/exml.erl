...
-define(DEFAULT_OPTIONS, [{space, 2}, {format, true}, {prolog, ?XML_PROLOG}, {level, 0}]).
...
export(X, Options) when is_tuple(X), is_list(Options) ->
    FinalOptions = maps:from_list(elist:merge_key_list(1, Options, ?DEFAULT_OPTIONS)),
...
xml_node({Tag, Attrs, [Content]}, Options) when not is_tuple(Content) ->
    xml_node({Tag, Attrs, eutil:to_binary(Content)}, Options);
xml_node({Tag, Attrs, Content}, #{space := Space, format := Format, level := Level} = Options) when is_list(Content) ->
    TagBin = eutil:to_binary(Tag),
...
xml_node({Tag, Attrs, Content},
    ...
    AttrsBin = attrs(Attrs),
    Node =
        case eutil:to_binary(Content) of
            <<"">> -> <<"<", TagBin/binary, AttrsBin/binary, "/>">>;
...
xml_node(Content, #{space := Space, format := Format, level := Level}) ->
    ContentBin = eutil:to_binary(Content),
    case Format of
        true ->
            SpaceBin = ebinary:repeat(<<" ">>, Level * Space),
...
attrs([{Attr, Value} | Rest], Acc) ->
    AttrBin = eutil:to_binary(Attr),
    ValueBin = eutil:to_binary(Value),
