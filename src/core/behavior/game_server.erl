...
%% 封装处理try ... catch
-define(DO_TRY(FunCode, Mod, DoArgs, State),
    try
        case (FunCode) of
            ?ok ->
                {?noreply, {Mod, State}};
...
%% @doc 收到事件消息时调用
-callback do_event(Signal :: #signal{}, State :: term()) ->
    {?noreply, NewState :: term()} |
    {?stop, Reason :: term(), NewState :: term()}.
...
handle_call({?EXEC_CALL, M, F, A} = Request, From, {Mod, State}) ->
    ?DO_TRY(M:F(State, A), Mod, {Request, From}, State);
handle_call(Request, From, {Mod, State}) ->
    ?DO_TRY(Mod:do_call(Request, From, State), Mod, {do_call, Request, From}, State).

handle_cast({?EXEC_CAST, M, F, A} = Request, {Mod, State}) ->
    ?DO_TRY(M:F(State, A), Mod, Request, State);
handle_cast({?stop, Reason}, {Mod, State}) ->
    NewState = stop(Mod, State, Reason),
    {?stop, Reason, {Mod, NewState}};
handle_cast(Request, {Mod, State}) ->
    ?DO_TRY(Mod:do_cast(Request, State), Mod, {do_cast, Request}, State).

handle_info(?loop, {Mod, State}) ->
    ?DO_TRY(Mod:do_loop(State), Mod, do_loop, State);
handle_info(#signal{} = Signal, {Mod, State}) ->
    ?DO_TRY(Mod:do_event(State), Mod, {do_event, Signal}, State);
handle_info(Info, {Mod, State}) ->
    ?DO_TRY(Mod:do_info(Info, State), Mod, {do_info, Info}, State).
